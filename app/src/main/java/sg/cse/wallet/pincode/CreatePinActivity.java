...
public class CreatePinActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private boolean isConfirmPin; // Màn hình ConfirmPin
    private String pinCode;  //Pin khởi tạo
    ...
    public void checkPin() {
        //Nếu là màn hình Confirm Pin
        if (isConfirmPin) {
            String confirmPin = edtPincode.getText().toString();
            if (confirmPin.equals(pinCode)) {
                Prefs.getInstance().savePinCode(pinCode);
                finish();
            } else {
